...
class AdaptiveEngineModelsTest {
    ...
    fun engineConfigDefaultsMatchThePythonReferenceExactly() {
        ...
        assertEquals(14, config.minimumHistoryDays.toLong())
        assertEquals(7, config.coverageWindowDays.toLong())
        assertEquals(6, config.minimumNutritionDaysPerWeek.toLong())
        assertEquals(1, config.minimumWeightDaysPerWeek.toLong())
